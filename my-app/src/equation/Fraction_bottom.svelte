...
let tree_props  
let tree_type
let arrow_type = 'bottom_i'
let arrow_left = 0 
let arrow_y = -12

$:if( tree && (!tree.updateProps || tree.full_name ) )
{
	// tree.updateFraction = ( caller=false) => {
...
$:if( tree ) 
{
	...
	tree.props.arrow_type = arrow_type
...
function arrow_mouse_down(e)
{
	...
	$clone = { 
		...
		drag_elem: this_bottom,
		arrow_type: 'bottom_blue' 
		}
}
...
function keyup(e)
{
	let { width } = this_bottom.getBoundingClientRect()
...
onMount( () => {
	...
	if( tree_props )
	{
		...
		if(tree_props.store_in_ref_elem)
		{
			...
			$clone.ref_elem.push( this_bottom )
...
{#if !isEditable}
	<div 
	bind:this={this_bottom}
	id={tree_name}
	class="bottom {arrow_type}"
